...
var TEXTEXT=["js","ts","py","java","c","cpp","cs","go","rs","rb","php","html","css","json","yml","yaml","sh","sql","md","txt","kt","swift","dart","xml","vue","jsx","tsx","toml","ini","cfg"];
...
function renderCtxInfo(){var n=state.context.length;var t=$("ctxinfo");if(n){t.textContent="("+n+" item \u00d7)";t.title="Klik buat kosongkan"}else{t.textContent=($("ctx").checked&&state.blocks.length)?"(halaman aktif)":"";t.title=""}}
...
function extOfName(n){var p=String(n).split(".");return p.length>1?p.pop().toLowerCase():""}
function extFromPath(p){var e=extOfName(p);return e||"plain text"}
...
async function loadGhTree(){if(window.innerWidth>900)document.body.classList.add("shownotion");$("page").innerHTML='<p class="muted">Memuat repo...</p>';try{var r=await fetch("/api/gh/tree");var d=await r.json();if(d.error){$("page").innerHTML='<p class="muted">'+esc(d.error)+'</p>';toast(d.error,true);return}var box=$("page");box.innerHTML='<div class="title">\ud83d\udcbb '+esc(d.repo)+' ('+esc(d.branch)+')</div>';if(!d.files.length){box.innerHTML+='<p class="muted">Repo kosong.</p>';return}var list=el("div","plist");d.files.forEach(function(f){var it=el("div","pitem","\ud83d\udcc4 "+esc(f.path));it.onclick=function(){openGhFile(f.path)};list.appendChild(it)});box.appendChild(list)}catch(e){toast("Gagal load repo",true)}}
async function openGhFile(path){$("page").innerHTML='<p class="muted">Memuat '+esc(path)+'...</p>';try{var r=await fetch("/api/gh/file?path="+encodeURIComponent(path));var d=await r.json();if(d.error){toast(d.error,true);return}renderGhFile(d)}catch(e){toast("Gagal buka file",true)}}
function renderGhFile(d){var box=$("page");box.innerHTML="";var bk=el("button","btn","\u2190 Daftar file");bk.onclick=loadGhTree;box.appendChild(bk);box.appendChild(el("div","title","\ud83d\udcc4 "+esc(d.path)));var card=el("div","codecard");var head=el("div","codehead");var sp=el("span","sp",esc(d.path));head.appendChild(sp);var ctxb=el("button","btn","\u2795 Konteks");var save=el("button","btn","\ud83d\udcbe Commit");head.appendChild(ctxb);head.appendChild(save);var ta=el("textarea");ta.value=d.content;card.appendChild(head);card.appendChild(ta);box.appendChild(card);save.onclick=function(){var msg=prompt("Pesan commit:","Update "+d.path+" via notionku");if(msg==null)return;ghCommit([{path:d.path,content:ta.value,sha:d.sha}],msg)};ctxb.onclick=function(){state.context=state.context.filter(function(x){return x.id!=="gh:"+d.path});state.context.push({id:"gh:"+d.path,title:d.path,codes:[{language:extFromPath(d.path),text:ta.value,path:d.path}]});renderCtxInfo();toast("Konteks + "+d.path)}}
async function ghCommit(files,msg){toast("Commit ke GitHub...");try{var r=await fetch("/api/gh/commit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({files:files,message:msg})});var d=await r.json();if(d.error){toast(d.error,true);return}var ok=(d.results||[]).filter(function(x){return x.ok}).length;var fail=(d.results||[]).length-ok;toast("Commit: "+ok+" sukses"+(fail?(", "+fail+" gagal"):""),fail>0);(d.results||[]).forEach(function(x){if(!x.ok)console.error("Commit gagal "+x.path+": "+x.error)})}catch(e){toast("Gagal commit",true)}}
function openSettings(){fetch("/api/settings").then(function(r){return r.json()}).then(function(d){if(d.error){toast(d.error,true);return}$("settokens").value="";$("settokinfo").textContent=(d.tokens&&d.tokens.length)?("Tersimpan: "+d.tokens.join(", ")+". Isi ulang buat ganti."):"Belum ada token.";$("setkey").value="";$("setkey").placeholder=d.openai_key_set?"(tersimpan) isi buat ganti":"sk-...";$("setbase").value=d.openai_base||"";$("setmodel").value=d.openai_model||"";$("setai").value=d.ai_model||"";$("setgh").value="";$("setgh").placeholder=d.gh_token_set?"(tersimpan) isi buat ganti":"ghp_...";$("setghowner").value=d.gh_owner||"";$("setghrepo").value=d.gh_repo||"";$("setghbranch").value=d.gh_branch||"";$("setmodal").style.display="flex"}).catch(function(){toast("Gagal load pengaturan",true)})}
function closeSettings(){$("setmodal").style.display="none"}
async function saveSettings(){var body={openai_base:$("setbase").value,openai_model:$("setmodel").value,ai_model:$("setai").value};var tk=$("settokens").value.trim();if(tk)body.tokens=tk.split("\n").map(function(x){return x.trim()}).filter(function(x){return x});var key=$("setkey").value.trim();if(key)body.openai_key=key;body.gh_owner=$("setghowner").value;body.gh_repo=$("setghrepo").value;body.gh_branch=$("setghbranch").value;var ght=$("setgh").value.trim();if(ght)body.gh_token=ght;$("setmsg").textContent="Menyimpan...";try{var r=await fetch("/api/settings/save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});var d=await r.json();if(d.error){$("setmsg").textContent=d.error;return}$("setmsg").textContent="Tersimpan \u2713";toast("Pengaturan disimpan");loadAccounts().then(doSearch);loadModels();setTimeout(closeSettings,700)}catch(e){$("setmsg").textContent="Gagal simpan"}}
