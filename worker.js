...
<body>
<div id="backdrop"></div>
<aside id="histbar"><div class="histtop"><span class="brand">🧑‍💻 Notion Coding</span></div><button class="newbtn" id="newchat">＋  Chat baru</button><div id="histlist"></div></aside>
<main class="center">
...
var state={pages:[],current:null,title:"",blocks:[],chat:[],chatId:null,memory:[]};
var editing=-1;var pending=[];
...
async function exportZip(){var codes=collectCode();if(!codes.length){toast("Nggak ada kode di halaman ini",true);return}var zip=new JSZip();var seen={};codes.forEach(function(c,i){var base="code_"+(i+1);var fn=base+"."+extOf(c.language);seen[fn]=(seen[fn]||0)+1;if(seen[fn]>1)fn=base+"_"+seen[fn]+"."+extOf(c.language);zip.file(fn,c.text)});var blob=await zip.generateAsync({type:"blob"});downloadFile((state.title||"notion").replace(/[^a-z0-9]/gi,"_")+".zip",blob)}
...
function showMemory(){var txt=state.memory.length?state.memory.join("\n"):"(kosong)";var nv=prompt("Memori AI (satu fakta per baris). Kosongkan buat hapus semua:",state.memory.join("\n"));if(nv==null)return;state.memory=nv.split("\n").map(function(x){return x.trim()}).filter(function(x){return x});saveMemory();toast("Memori disimpan")}
...
function collectCodeFromText(text){var out=[];var lines=String(text).split("\n");var i=0;while(i<lines.length){if(lines[i].indexOf(FENCE)===0){var lang=lines[i].slice(3).trim()||"plain text";var buf=[];i++;while(i<lines.length&&lines[i].indexOf(FENCE)!==0){buf.push(lines[i]);i++}i++;out.push({language:lang,text:buf.join("\n")})}else i++}return out}
async function zipFromText(text,base){var codes=collectCodeFromText(text);if(!codes.length){toast("Nggak ada kode",true);return}var zip=new JSZip();var seen={};codes.forEach(function(c,i){var fn="file_"+(i+1)+"."+extOf(c.language);seen[fn]=(seen[fn]||0)+1;if(seen[fn]>1)fn="file_"+(i+1)+"_"+seen[fn]+"."+extOf(c.language);zip.file(fn,c.text)});var blob=await zip.generateAsync({type:"blob"});downloadFile((base||"output")+".zip",blob)}
...
async function zipAll(){var codes=collectCode();var fromChat=[];state.chat.forEach(function(m){if(m.role==="assistant")fromChat=fromChat.concat(collectCodeFromText(m.content))});var all=codes.concat(fromChat);if(!all.length){toast("Nggak ada kode buat di-zip",true);return}var zip=new JSZip();var seen={};all.forEach(function(c,i){var fn="file_"+(i+1)+"."+extOf(c.language);seen[fn]=(seen[fn]||0)+1;if(seen[fn]>1)fn="file_"+(i+1)+"_"+seen[fn]+"."+extOf(c.language);zip.file(fn,c.text)});var blob=await zip.generateAsync({type:"blob"});downloadFile("semua-kode.zip",blob);toast("ZIP berisi "+all.length+" file")}
...
function buildContext(){var sys="Kamu asisten coding yang membantu. Balas dalam bahasa Indonesia kecuali diminta lain. Untuk kode, selalu pakai blok kode markdown dengan nama bahasa. Kalau mau kasih pilihan ke user, tulis di baris terpisah diawali ::OPTIONS:: lalu opsi dipisah tanda |. Kalau ada fakta penting jangka panjang tentang user/proyek, tulis di baris diawali ::MEMORY:: .";if(state.memory.length)sys+="\nMemori: "+state.memory.join("; ");if($("ctx").checked&&state.blocks.length){var codes=collectCode();if(codes.length){sys+="\n\nKode dari halaman Notion aktif ('"+state.title+"'):\n";codes.forEach(function(c){sys+=FENCE+c.language+"\n"+c.text+"\n"+FENCE+"\n"})}}return sys}
function msgsForApi(){var arr=[{role:"system",content:buildContext()}];state.chat.forEach(function(m){var content=m.content;if(m.files&&m.files.length){m.files.forEach(function(f){if(f.kind==="text")content+="\n\n[File: "+f.name+"]\n"+f.text}); }arr.push({role:m.role,content:content})});return arr}
...
async function loadHistory(){try{var r=await fetch("/api/history");var d=await r.json();var box=$("histlist");box.innerHTML="";(d.sessions||[]).forEach(function(s){var it=el("div","hitem"+(state.chatId===s.id?" active":""));var t=el("div","t",esc(s.title));t.onclick=function(){openChat(s.id)};it.appendChild(t);var rn=el("button","a","\u270f\ufe0f");rn.onclick=function(ev){ev.stopPropagation();renameChat(s.id,s.title)};var dl=el("button","a","\ud83d\uddd1\ufe0f");dl.onclick=function(ev){ev.stopPropagation();delChatById(s.id)};it.appendChild(rn);it.appendChild(dl);box.appendChild(it)})}catch(e){}}
...
function routeFromUrl(){var m=location.pathname.match(/^\/chat\/(.+)$/);if(m){var id=decodeURIComponent(m[1]);fetch("/api/history?id="+encodeURIComponent(id)).then(function(r){return r.json()}).then(function(d){if(d&&!d.error){state.chatId=d.id;state.chat=d.messages||[];renderChat();loadHistory()}})}}
...
$("find").addEventListener("input",function(){var v=this.value.trim();clearTimeout(window.__ft);var self=this;window.__ft=setTimeout(function(){findInPage(v)},250)});
