...
async function openSettings(){var m=$("setmodal");m.style.display="flex";$("setmsg").textContent="";try{var r=await fetch("/api/settings");var d=await r.json();if(d.error){$("setmsg").textContent=d.error;return}$("settokens").value="";$("settokinfo").textContent=(d.tokens&&d.tokens.length)?("Tersimpan: "+d.tokens.join(", ")+". Isi ulang buat ganti."):"Belum ada token tersimpan.";$("setkey").value="";$("setkey").placeholder=d.openai_key_set?"(tersimpan — kosongin kalau nggak diubah)":"sk-...";$("setbase").value=d.openai_base||"";$("setmodel").value=d.openai_model||"";$("setai").value=d.ai_model||""}catch(e){$("setmsg").textContent="Gagal load settings"}}
function closeSettings(){$("setmodal").style.display="none"}
async function saveSettings(){$("setmsg").textContent="Menyimpan...";var body={openai_base:$("setbase").value.trim(),openai_model:$("setmodel").value.trim(),ai_model:$("setai").value.trim()};var tl=$("settokens").value.split("\n").map(function(s){return s.trim()}).filter(function(s){return s});if(tl.length)body.tokens=tl;var key=$("setkey").value.trim();if(key)body.openai_key=key;try{var r=await fetch("/api/settings/save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});var d=await r.json();if(d.error){$("setmsg").textContent=d.error;return}$("setmsg").textContent="Tersimpan ✅ ("+(d.tokenCount||0)+" token)";loadAccounts().then(doSearch);setTimeout(closeSettings,800)}catch(e){$("setmsg").textContent="Gagal simpan"}}
