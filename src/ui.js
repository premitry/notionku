...
<div id="setmodal"><div class="modalcard"><h2 style="margin:0 0 4px;font-size:18px">⚙️ Pengaturan</h2><p class="muted" style="margin:0 0 14px;font-size:12px">Semua disimpan di Cloudflare KV.</p><div class="field"><span class="lbl">Notion Tokens (opsional — buat akses konten Notion, satu per baris)</span><textarea id="settokens" class="set" rows="4" placeholder="ntn_xxx (opsional)"></textarea><div class="row" style="justify-content:space-between;margin-top:4px"><span id="settokinfo" class="muted" style="font-size:11px"></span><button class="btn" id="setdeltok" type="button" style="font-size:11px;padding:3px 8px" onclick="if(confirm('Hapus semua Notion token yang tersimpan?')){var b=this;b.disabled=true;fetch('/api/settings/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({clear_tokens:true})}).then(function(r){return r.json()}).then(function(d){b.disabled=false;var m=document.getElementById('setmsg');if(d&&d.error){if(m)m.textContent=d.error;return}var t=document.getElementById('settokens');if(t)t.value='';var i=document.getElementById('settokinfo');if(i)i.textContent='Belum ada token.';if(m)m.textContent='Token dihapus';}).catch(function(){b.disabled=false;var m=document.getElementById('setmsg');if(m)m.textContent='Gagal hapus';})}">🗑️ Hapus token</button></div></div><div class="field"><span class="lbl">OpenAI API Key</span><input id="setkey" class="search" type="password" placeholder="sk-..."/></div><div class="field"><span class="lbl">OpenAI Base URL (opsional)</span><input id="setbase" class="search" placeholder="https://api.openai.com/v1"/></div><div class="field"><span class="lbl">Default Model (opsional)</span><input id="setmodel" class="search" placeholder="gpt-4o-mini"/></div><div class="field"><span class="lbl">Workers AI Model (opsional)</span><input id="setai" class="search" placeholder="@cf/meta/llama-3.1-8b-instruct"/></div><div class="field"><span class="lbl">GitHub — Connect (OAuth, tanpa token manual)</span><input id="setghclient" class="search" placeholder="OAuth App Client ID (mis. Iv1.xxx / Ov23...)"/><div class="row" style="gap:6px;margin-top:2px"><button class="btn accent" id="ghconnect" type="button">🔗 Connect GitHub</button><span id="ghconnstat" class="muted" style="font-size:11px"></span></div><div id="ghconnbox" class="muted" style="font-size:12px;margin-top:6px"></div><select id="setghrepo2" class="search" style="margin-top:6px;display:none"></select><p class="muted" style="font-size:11px;margin:4px 0 0">Sekali connect, repo tinggal dipilih dari dropdown. Bikin Client ID: GitHub Settings → Developer settings → OAuth Apps → New, aktifkan "Enable Device Flow".</p></div><div class="field"><span class="lbl">GitHub Token (scope: repo)</span><input id="setgh" class="search" type="password" placeholder="ghp_..."/></div><div class="field"><span class="lbl">GitHub owner / repo / branch</span><div class="row"><input id="setghowner" class="search" placeholder="owner" style="margin:0"/><input id="setghrepo" class="search" placeholder="repo" style="margin:0"/><input id="setghbranch" class="search" placeholder="main" style="margin:0;max-width:90px"/></div></div><div class="field"><label class="row" style="align-items:flex-start;gap:6px"><input type="checkbox" id="setautopush" style="margin-top:3px"/> <span>Auto-push ke GitHub (tanpa pencet Commit)</span></label><p class="muted" style="font-size:11px;margin:4px 0 0">Kalau nyala, tiap balasan AI yang ada file-nya (blok kode dengan path) langsung di-commit otomatis - semua file jadi 1 commit rapi. Perlu GitHub tersambung + owner/repo terisi.</p></div><div class="field"><span class="lbl">Backup data (chat + memori)</span><div class="row"><button class="btn" id="setexport">⬇️ Export JSON</button><button class="btn" id="setimport">⬆️ Import JSON</button><input type="file" id="impfile" accept="application/json" style="display:none"/></div></div><div id="setmsg" class="muted" style="font-size:12px;min-height:16px"></div><div class="row" style="margin-top:10px;justify-content:flex-end"><button class="btn" id="setclose">Tutup</button><button class="btn accent" id="setsave">Simpan</button></div></div></div>
<div id="toast"></div>
`;
